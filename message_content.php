...
$message_No = $_GET['No'];
$SQL = "SELECT 留言.舍監編號 as 舍監編號, 標題, 留言.學號, 內容, 日期, 姓名, 回覆內容, 回覆時間
        FROM 留言, 學生
...
<div class="card border-secondary">
    ...
    <?php
    if ($row["回覆內容"] == null) {     //未回覆 AND 是"舍監"：顯示新增回復btn
        if (isset($_SESSION["login_identity"]) && $_SESSION["login_identity"] == "舍監") {
    ?>
            <butoon class='btn btn-secondary' style='float: right;' data-bs-toggle='modal' data-bs-target='#message_reply_model'>
                <i class='bi bi-chat-dots-fill'></i>&nbsp; 回覆留言
            </butoon>
            <!-- 新增回覆 Modal -->
        ...
        <div class="card-footer">
            <p style="text-align: end;" class="card-text">
                <?php
                echo $row['回覆時間'];
                echo "&nbsp;&nbsp;<i class='bi bi-person-fill'></i> " . $row['舍監編號'];
                ?>
            </p>
        </div>
</div>
